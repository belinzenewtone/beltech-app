...
void main() {
  ...
  group('EncryptedExportService', () {
    test('encrypt produces non-empty output different from plaintext', () {
      ...
      final encrypted = service.encrypt(plainText: plain, password: 'secret123');
    ...
    test('decrypt with correct password returns original plaintext', () {
      const plain = 'id,title,amount\n1,Food,500\n2,Transport,300';
      final encrypted = service.encrypt(plainText: plain, password: 'my_password');
      final decrypted = service.decrypt(base64Payload: encrypted, password: 'my_password');
    ...
    test('handles empty plaintext', () {
      ...
      final decrypted = service.decrypt(base64Payload: encrypted, password: 'pw');
      expect(decrypted, '');
    });

    test('handles large plaintext', () {
      final plain = List.generate(100, (i) => 'row $i, data, ${i * 10}').join('\n');
      final encrypted = service.encrypt(plainText: plain, password: 'secure_pass');
      final decrypted = service.decrypt(base64Payload: encrypted, password: 'secure_pass');
      expect(decrypted, plain);
    });
  });
}
