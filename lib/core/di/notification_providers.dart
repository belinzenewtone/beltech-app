...
final learningRemindersEnabledProvider = FutureProvider<bool>(
  (ref) =>
      ref.watch(localNotificationServiceProvider).isLearningRemindersEnabled(),
);

final budgetAlertThresholdsProvider = FutureProvider<(double, double, double)>(
  (ref) =>
      ref.watch(localNotificationServiceProvider).getBudgetAlertThresholds(),
);
...
class NotificationPreferenceController extends AsyncNotifier<void> {
  ...
  Future<void> setLearningRemindersEnabled(bool enabled) async {
    ...
    state = await AsyncValue.guard(() async {
      await ref
          .read(localNotificationServiceProvider)
          .setLearningRemindersEnabled(enabled);
      ref.invalidate(learningRemindersEnabledProvider);
    });
  }
  ...
  Future<void> setLearningReminderTime(int hour, int minute) async {
    ...
    state = await AsyncValue.guard(() async {
      ...
      await ref
          .read(revampTelemetryServiceProvider)
          .track(
            'learning_reminder_time_changed',
            attributes: {'hour': hour, 'minute': minute},
          );
