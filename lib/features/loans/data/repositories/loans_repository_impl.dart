...
class LoansRepositoryImpl implements LoansRepository {
  ...
  Future<void> addLoan({
    ...
    await _store.executor.runInsert(
      ...
      [
        name, lender, totalAmount, outstandingAmount, interestRate,
        startDate?.millisecondsSinceEpoch, dueDate?.millisecondsSinceEpoch,
        status.name, DateTime.now().millisecondsSinceEpoch,
      ],
    );
    _store.emitChange();
  }
  ...
  Future<void> updateLoan({
    ...
    final sets = <String>[];
    final args = <Object?>[];
    if (name != null) { sets.add('name = ?'); args.add(name); }
    if (lender != null) { sets.add('lender = ?'); args.add(lender); }
    if (totalAmount != null) { sets.add('total_amount = ?'); args.add(totalAmount); }
    if (outstandingAmount != null) { sets.add('outstanding_amount = ?'); args.add(outstandingAmount); }
    if (interestRate != null) { sets.add('interest_rate = ?'); args.add(interestRate); }
    if (startDate != null) { sets.add('start_date = ?'); args.add(startDate.millisecondsSinceEpoch); }
    if (dueDate != null) { sets.add('due_date = ?'); args.add(dueDate.millisecondsSinceEpoch); }
    if (status != null) { sets.add('status = ?'); args.add(status.name); }
    if (sets.isEmpty) return;
    args.add(id);
    await _store.executor.runUpdate(
      'UPDATE loans SET ${sets.join(', ')} WHERE id = ?', args,
    );
    _store.emitChange();
  }
  ...
  LoanItem _rowToLoan(Map<String, Object?> row) {
    return LoanItem(
      ...
      interestRate: row['interest_rate'] != null ? _asDouble(row['interest_rate']) : null,
