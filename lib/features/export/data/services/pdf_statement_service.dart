...
class PdfStatementService {
        ...
        children: [
          ...
          pw.Padding(
            ...
            child: pw.Table(
              ...
              children: [
                _summaryRow('Total Income', '+KES ${amountFmt.format(totalIncome)}', PdfColors.green700),
                _summaryRow('Total Expenses', '-KES ${amountFmt.format(totalExpenses)}', PdfColors.red700),
                _summaryRow('Net Balance', 'KES ${amountFmt.format(netBalance)}',
                    netBalance >= 0 ? PdfColors.green900 : PdfColors.red900),
                _summaryRow('Transaction Count', count.toString(), PdfColors.black),
              ],
            ),
          ),
        ],
  ...
  pw.Widget _buildTransactionTable(
    ...
    return pw.TableHelper.fromTextArray(
      ...
      headers: headers,
      data: rows
          .map((r) => [r.date, r.title, r.category, r.amount])
          .toList(),
      border: pw.TableBorder.all(color: PdfColors.grey200, width: 0.5),
      columnWidths: {
