...
class AnomalyDetector {
  ...
  List<Anomaly> _detectDuplicates(List<RecentTransaction> txs) {
    ...
    for (final tx in txs) {
      final key = '${tx.title.trim().toLowerCase()}_${tx.amount.toStringAsFixed(2)}_${tx.date.day}';
      if (seen.containsKey(key)) {
        anomalies.add(Anomaly(
          type: AnomalyType.duplicate,
          description: 'Duplicate transaction: "${tx.title}" for ${tx.amount.toStringAsFixed(0)}',
  ...
  List<Anomaly> _detectDailySurges(List<RecentTransaction> txs) {
    ...
    for (final tx in txs.where((t) => t.type == 'expense')) {
      final day = DateTime(tx.date.year, tx.date.month, tx.date.day).millisecondsSinceEpoch ~/ 86400000;
    ...
    return dailyTotals.entries
        ...
        .map((e) => Anomaly(
              ...
              description: 'Unusually high spending day: ${e.value.toStringAsFixed(0)}',
              severity: e.value > threshold * 1.5 ? AnomalySeverity.high : AnomalySeverity.medium,
            ))
        .toList();
  }
  ...
  List<Anomaly> _detectCategorySpikes(List<RecentTransaction> txs) {
    ...
    for (final tx in txs.where((t) => t.type == 'expense')) {
      byCategory.putIfAbsent(tx.category.toLowerCase(), () => []).add(tx.amount);
    }
    final anomalies = <Anomaly>[];
    for (final entry in byCategory.entries) {
      ...
      if (latest > mean * 3 && latest > 1000) {
        anomalies.add(Anomaly(
          ...
          description: 'Spike in ${entry.key}: ${latest.toStringAsFixed(0)} vs avg ${mean.toStringAsFixed(0)}',
          severity: latest > mean * 5 ? AnomalySeverity.high : AnomalySeverity.medium,
        ));
      }
    }
    return anomalies;
  ...
  List<Anomaly> _detectUnusualAmounts(List<RecentTransaction> txs) {
    ...
    final amounts = txs.where((t) => t.type == 'expense').map((t) => t.amount).toList();
    if (amounts.length < 3) return [];
    final mean = amounts.reduce((a, b) => a + b) / amounts.length;
    final variance = amounts.map((v) => pow(v - mean, 2)).reduce((a, b) => a + b) / amounts.length;
    final stdDev = sqrt(variance);
    final threshold = mean + 2.5 * stdDev;

    return txs
        .where((t) => t.type == 'expense' && t.amount > threshold && t.amount > mean * 2)
        .map((t) => Anomaly(
              type: AnomalyType.unusualAmount,
              description: 'Unusual amount: "${t.title}" ${t.amount.toStringAsFixed(0)}',
              severity: t.amount > threshold * 1.5 ? AnomalySeverity.high : AnomalySeverity.medium,
            ))
        .toList();
  }
}
