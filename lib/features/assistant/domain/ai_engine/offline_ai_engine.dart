...
class OfflineAiEngine {
  ...
  Future<String> reply({
    ...
    return _generateReply(intent, query, ctx,
  ...
  String _generateReply(
    ...
    switch (intent) {
      ...
      case Intent.anomalyAlert:
        ...
        if (anomalies.isEmpty) return 'No anomalies detected. Your data looks clean.';
        final high = anomalies.where((a) => a.severity == AnomalySeverity.high).length;
        final lines = anomalies.take(3).map((a) => '• ${a.description}').join('\n');
      ...
      case Intent.goalSummary:
        ...
        final lines = ctx.goals.take(3).map((g) {
  ...
  String _healthAdvice(int score, DataContext ctx) {
    if (score >= 80) return 'Keep it up! Your savings rate and task discipline are strong.';
    if (score >= 60) {
      if (ctx.billsOverdue.isNotEmpty) return 'Consider clearing overdue bills to improve your score.';
      if (ctx.loansOutstanding > 0) return 'Try reducing loan burden to boost your health.';
      return 'You are doing okay. Small improvements in savings will help.';
    }
    if (ctx.overdueTasksCount > 0) return 'Address overdue tasks and reduce spending spikes.';
  ...
  String _financialAdvice(DataContext ctx) {
    ...
    if (ctx.monthIncome > 0) {
      ...
      if (rate < 0) tips.add('Your expenses exceed income. Prioritize essential spending.');
    }
    if (ctx.billsUpcoming.isNotEmpty) {
      tips.add('You have ${ctx.billsUpcoming.length} upcoming bill(s). Plan ahead.');
    }
    if (ctx.goals.isNotEmpty) {
      final atRisk = ctx.goals.where((g) => g.atRisk).length;
      if (atRisk > 0) tips.add('$atRisk goal(s) are at risk. Consider increasing contributions.');
    }
    if (tips.isEmpty) return 'You are on a good path. Keep tracking consistently.';
    return tips.join(' ');
  }
  ...
  String _merchantReply(List<RecentTransaction> txs) {
    ...
    final top = sorted.take(3).map((e) => '• ${e.key}: ${CurrencyFormatter.money(e.value)}').join('\n');
  ...
  String _categoryReply(DataContext ctx) {
    ...
    final lines = ctx.topCategories.take(5).map((c) => '• ${c.$1}: ${CurrencyFormatter.money(c.$2)}').join('\n');
