...
class GenerateSpendInsightsUseCase {
  ...
  List<SpendInsight> generateInsights({
    ...
    final expenseDescriptions = thisWeekExpenses
    ...
    for (final rule in dueRecurring) {
      final isFound = expenseDescriptions.any(
        (desc) =>
            desc.contains(rule.name.toLowerCase()) ||
            desc.contains(rule.merchant.toLowerCase()),
      );
    ...
    if (highFeeExpenses.isNotEmpty) {
      final totalFees = highFeeExpenses.fold<double>(
        0,
        (sum, e) => sum + (e.fee ?? 0),
      );
      final avgFeePercent =
          (totalFees / _sumExpenseAmount(highFeeExpenses)) * 100;
  ...
  Map<String, double> _aggregateByMerchant(List<Expense> expenses) {
    ...
    for (final expense in expenses) {
      result[expense.merchant] =
          (result[expense.merchant] ?? 0.0) + expense.amount;
    }
    return result;
  }
