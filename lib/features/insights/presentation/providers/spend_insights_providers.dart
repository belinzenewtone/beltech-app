...
final generateSpendInsightsUseCaseProvider = Provider(
  (ref) => const GenerateSpendInsightsUseCase(),
);
...
final spendInsightsProvider = FutureProvider<List<SpendInsight>>((ref) async {
  ...
  Expense toExpense(ExpenseItem e) => Expense(
    id: e.id.toString(),
    amount: e.amountKes,
    merchant: e.title,
    description: e.title,
  ...
  RecurringRule toRule(RecurringTemplate t) => RecurringRule(
    id: t.id.toString(),
    name: t.title,
  ...
  final recurringRules = templates.map(toRule).toList();
