...
class CsvImportService {
  ...
  Map<String, String> _detectMappings(List<String> headers) {
    ...
    for (final header in headers) {
      ...
      if (!seenTargets.contains('occurred_at') && _isDateColumn(normalized)) {
        ...
        target = 'title';
      } else if (!seenTargets.contains('category') && _isCategoryColumn(normalized)) {
        target = 'category';
      } else if (!seenTargets.contains('amountKes') && _isAmountColumn(normalized)) {
        target = 'amountKes';
      } else if (!seenTargets.contains('balanceAfterKes') && _isBalanceColumn(normalized)) {
  ...
  bool _isDateColumn(String normalized) {
    const patterns = [
      'date', 'occurred', 'occurred_at', 'occurredat', 'created',
      'timestamp', 'time', 'datetime', 'date_time', 'posting date',
      'transaction date', 'txn date', 'tx_date', 'txdate',
    ];
  ...
  bool _isTitleColumn(String normalized) {
    const patterns = [
      'description', 'desc', 'title', 'name', 'narration', 'narrative',
      'details', 'detail', 'memo', 'note', 'notes', 'particulars',
      'payee', 'merchant', 'recipient', 'sender',
    ];
    return patterns.any((p) => normalized == p || normalized.contains(p));
  }
  ...
  bool _isCategoryColumn(String normalized) {
    const patterns = [
      'category', 'cat', 'type', 'group', 'tag', 'class',
    ];
    return patterns.any((p) => normalized == p || normalized.contains(p));
  }
  ...
  bool _isAmountColumn(String normalized) {
    const patterns = [
      'amount', 'kes', 'value', 'price', 'cost', 'sum', 'total',
      'debit', 'credit', 'withdrawal', 'deposit', 'paid', 'paid out',
      'money out', 'money in', 'transaction amount',
    ];
    return patterns.any((p) => normalized == p || normalized.contains(p));
  }
  ...
  bool _isBalanceColumn(String normalized) {
    const patterns = [
      'balance', 'bal', 'balance_after', 'balanceafter', 'balance after',
