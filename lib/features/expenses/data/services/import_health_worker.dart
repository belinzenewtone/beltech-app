...
class ImportHealthWorker {
  ...
  ImportHealthReport analyzeHealth({
    ...
    final mediumConfidence =
        confidenceScores.where((s) => s >= 0.50 && s < 0.75).length;
    final lowConfidence = confidenceScores.where((s) => s < 0.50).length;

    final avgConfidence =
        confidenceScores.isEmpty ? 0.0 : confidenceScores.reduce((a, b) => a + b) / confidenceScores.length;
  ...
  String? _generateAlert(double quarantinePercent, double avgConfidence, int queueDepth) {
