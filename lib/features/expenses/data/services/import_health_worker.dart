...
class ImportHealthWorker {
  ...
  ImportHealthReport analyzeHealth({
    ...
    final highConfidence = confidenceScores.where((s) => s >= 0.85).length;
    final mediumConfidence = confidenceScores
        .where((s) => s >= 0.60 && s < 0.85)
        .length;
    final lowConfidence = confidenceScores.where((s) => s < 0.60).length;
    ...
    final isHealthy = quarantinePercent < 20 && avgConfidence > 0.75;
