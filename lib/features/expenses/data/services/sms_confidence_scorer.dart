...
class SmsConfidenceScorer {
  ...
  SmsConfidenceAnalysis scoreTransaction({
    ...
    final duplicateScore = _scoreDuplicateLikelihood(candidate, recentTransactions);
  ...
  double _scoreMerchantClarity(ParsedMpesaCandidate candidate) {
    final hasCounterparty = candidate.counterparty != null &&
        candidate.counterparty!.isNotEmpty &&
        candidate.counterparty!.length > 2;

    final hasReason = candidate.reason != null &&
  ...
  double _scoreDuplicateLikelihood(
    ...
    final exactDuplicates = last24h.where((t) {
      final sameAmount = (t.amountKes - candidate.amountKes).abs() <= amountTolerance;
...
class SmsConfidenceAnalysis {
  ...
  @override
  String toString() => 'SmsConfidenceAnalysis(score: $score, confidence: $confidence)';
}
