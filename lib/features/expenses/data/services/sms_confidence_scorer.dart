...
enum SenderTrust {
  ...
  static SenderTrust fromSender(String? sender) {
    if (sender == null || sender.isEmpty) return SenderTrust.unknown;
    final lower = sender.toLowerCase();
    if (lower.contains('mpesa')) return SenderTrust.officialMpesa;
    if (_looksLikeBank(lower)) return SenderTrust.bank;
    return SenderTrust.unknown;
  }

  static bool _looksLikeBank(String lower) {
    if (RegExp(r'^\d+$').hasMatch(lower)) return true;
    const banks = [
      'kcb', 'equity', 'coop', 'absa', 'ncba', 'stanbic', 'barclays',
      'family', 'dtb', 'sbm', 'habib', 'gulf', 'postbank', 'kwft',
      'unaitas', 'm-shwari', 'hustler',
    ];
    return banks.any(lower.contains);
  }
}
...
class SmsConfidenceScorer {
  /// Compute a confidence level from individual extraction signals.
  ///
  /// Weights (sum = 1.0):
  ///   code 15 % · amount 20 % · date 15 % · merchant 20 % · type 15 % · trust 15 %
  ///
  /// Thresholds: ≥ 0.85 → high · ≥ 0.60 → medium · < 0.60 → low
  MpesaConfidence score({
    required bool hasRealCode,
    ...
    required int matchedRulePhase,
  }) {
    final codeScore = hasRealCode ? 1.0 : 0.55;
    final amountScore = hasAmount ? 1.0 : 0.0;
    final dateScore = hasDate ? 1.0 : 0.4;
    final merchantScore = hasMerchant ? 1.0 : 0.2;
    final typeScore = _scoreTransactionType(transactionType);
    final trustScore = switch (senderTrust) {
      SenderTrust.officialMpesa => 1.0,
      SenderTrust.bank => 0.65,
      SenderTrust.unknown => 0.5,
    };
    final phaseBoost = switch (matchedRulePhase) {
      1 => 0.0,
      2 => -0.05,
      _ => -0.10,
    };
    final raw = codeScore * 0.15 +
        amountScore * 0.20 +
        dateScore * 0.15 +
        merchantScore * 0.20 +
        typeScore * 0.15 +
        trustScore * 0.15;
    return _scoreToConfidence((raw + phaseBoost).clamp(0.0, 1.0));
  }
  ...
  MpesaConfidence _scoreToConfidence(double score) {
    if (score >= 0.85) return MpesaConfidence.high;
    if (score >= 0.60) return MpesaConfidence.medium;
