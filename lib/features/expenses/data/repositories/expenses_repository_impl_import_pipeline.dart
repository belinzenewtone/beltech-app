...
Future<int> _processQueueChunk(
  ...
  final acceptedKeys = <String>{}; // O(1) within-batch dedup
  var imported = 0;
  ...
  for (final row in rows) {
    ...
    try {
      ...
      if (await _isDuplicateImpl(repo, candidate) ||
          _isDuplicateInChunk(candidate, acceptedCandidates, acceptedKeys)) {
      ...
      switch (candidate.route) {
        case MpesaParseRoute.directLedger:
          ...
          _addToAccepted(candidate, acceptedCandidates, acceptedKeys);
  ...
  for (final direct in directRows) {
    ...
    try {
      ...
      await _upsertPaybillAndFulizaImpl(repo, candidate);
      final mpesaCode =
          candidate.mpesaCode.isEmpty ? null : candidate.mpesaCode;
...
/// O(1) hash checks via [acceptedKeys], O(n) only for the heuristic fallback.
/// [acceptedKeys] is keyed by sourceHash, semanticHash, and `code:<mpesaCode>`.
bool _isDuplicateInChunk(
  ...
  if (candidate.mpesaCode.isNotEmpty &&
      acceptedKeys.contains('code:${candidate.mpesaCode}')) {
    return true;
  }
  if (acceptedKeys.contains(candidate.sourceHash)) return true;
  if (acceptedKeys.contains(candidate.semanticHash)) return true;
  // Heuristic: same merchant + same day + nearly same amount.
  for (final other in accepted) {
    ...
    if (sameTitle && sameDay && nearAmount) return true;
  }
  return false;
}
...
void _addToAccepted(
  ...
  if (candidate.mpesaCode.isNotEmpty) {
    acceptedKeys.add('code:${candidate.mpesaCode}');
  }
  acceptedKeys.add(candidate.sourceHash);
  acceptedKeys.add(candidate.semanticHash);
}
...
Future<void> _insertDirectImpl(
  ...
  await repo._store.addTransaction(
    ...
    mpesaCode: candidate.mpesaCode.isEmpty ? null : candidate.mpesaCode,
  );
  if (_shouldCreateIncome(candidate)) {
