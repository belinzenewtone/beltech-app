...
class ExpensesRepositoryImpl implements ExpensesRepository {
  ...
  Future<void> importRealtimeSmsMessage({
    ...
    // Drain the queue immediately while the app is in the foreground.
    unawaited(_processDueQueueImpl(this));
  }
  ...
  /// Enqueue [envelopes] into [sms_import_queue] without pre-parsing.
  ///
  /// Parsing happens once, later, inside [_processQueueChunk] (which runs in
  /// an [Isolate]).  Removing the pre-parse here eliminates the double-parse
  /// that previously occurred for every enqueue+process cycle.
  ///
  /// New rows land with conservative defaults (route='quarantine', confidence=0)
  /// that are overwritten with the real values during queue processing.
  /// Already-queued retry/failed rows are reset to 'pending' so they get
  /// re-processed.  A [SmsIngestionWorker] one-shot is scheduled as a durable
  /// backup in case the app is killed before [_processDueQueueImpl] runs.
  Future<void> _enqueueSmsImports(List<_QueuedSmsImport> envelopes) async {
    ...
    if (rawByHash.isEmpty) return;

    final allHashes = rawByHash.keys.toList();
    final alreadyImported = await _existingSourceHashes('transactions', allHashes);
    final alreadyQueued = await _existingSourceHashes('sms_import_queue', allHashes);

    final insertRows = <List<Object?>>[];
    final retryHashes = <String>[];

    for (final hash in rawByHash.keys) {
      if (alreadyImported.contains(hash)) continue;

      if (alreadyQueued.contains(hash)) {
        retryHashes.add(hash); // reset retry/failed rows to pending
      } else {
        final sourceTimestampMs = sourceTimestampByHash[hash]?.millisecondsSinceEpoch;
        insertRows.add([
          'local',
          rawByHash[hash]!,
          hash,
          hash, // semantic_hash placeholder; overwritten at parse time
          sourceTimestampMs,
          'pending',
          MpesaParseRoute.quarantine.name, // conservative default
          0.0,
          nowMs,
          nowMs,
        ]);
    ...
    if (retryHashes.isNotEmpty) {
      await _store.retryQueueRows(retryHashes);
    }

    // Schedule a durable one-shot worker as a crash-safety backstop.
    // The caller still invokes _processDueQueueImpl for immediate processing.
    SmsIngestionWorker.scheduleOneShot().ignore();
  }
