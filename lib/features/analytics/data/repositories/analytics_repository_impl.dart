...
class AnalyticsRepositoryImpl implements AnalyticsRepository {
  ...
  Future<AnalyticsSnapshot> _loadSnapshot(AnalyticsPeriod period) async {
    ...
    final eventsThisMonth = eventCountRows.isEmpty
        ? 0
        : _asInt(eventCountRows.first['total']);
    final productivity = _productivityScore(
      completed: completed,
      pending: pending,
    );
    ...
    return AnalyticsSnapshot(
      ...
      weeklySpending: weeklyMap.entries
          .map(
            (entry) => AnalyticsPoint(label: entry.key, amountKes: entry.value),
          )
          .toList(),
  ...
  double _productivityScore({required int completed, required int pending}) {
